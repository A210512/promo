...
const Cards = ({
  ...
  const commonCardJSX = (item: any, index: any) => (
    <div className='flex flex-col items-center ' key={index}>
      <div className='card-image'>
        <img src={item.img ? item.img : '/noimage.png'} alt={item.modelName} />
      </div>
      <div className='flex flex-col items-center h-auto md:h-auto'>
        <p className='cardsText lg:text-lg text-[10px] text-center h-auto w-[85%]'>
          {item.modelName}
        </p>
        <div className=''>
          ...
          <div className='w-full h-auto flex flex-row justify-center'>
            ...
            <div className='flex flex-wrap gap-1 xl:gap-2'>
              {item.sizeEU.map((size: any, index: number) => (
                <div
                  key={index}
                  className="left-[27px] top-[5px] text-black text-xs lg:fz-14 font-normal font-['Open Sans']  border border-neutral-200 leading-none p-1 "
                >
                  {size}
                </div>
              ))}
  ...
  return (
    <div id={name}>
      ...
      <a
        href={!showAllModels ? `#${name}` : undefined}
        className='w-auto md:w-[185px] h-12  bg-black mt-5 mx-[46px] md:mx-auto flex justify-center items-center '
        onClick={() => setShowAllModels(!showAllModels)}
      >
        <p className='text-center text-white text-sm font-semibold fursNeue leading-snug'>
          {' '}
